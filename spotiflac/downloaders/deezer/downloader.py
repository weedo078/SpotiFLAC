"""Main Deezer downloader implementation."""
...
from .client import DeezerAPIClient
...
class DeezerDownloader(BaseDownloader):
    ...
    Downloads tracks from Deezer by ISRC code, including metadata and cover art.
    ...
    def __init__(
        self,
        *,
        api_client: Optional[DeezerAPIClient] = None,
        metadata_writer: Optional[DeezerMetadataWriter] = None,
    ):
        """Initialize Deezer downloader.
        
        Args:
            api_client: Optional DeezerAPIClient instance
            metadata_writer: Optional DeezerMetadataWriter instance
        """
        self.api_client = api_client or DeezerAPIClient()
        self.metadata_writer = metadata_writer or DeezerMetadataWriter()
    ...
    def download_track(
        ...
        *,
        ...
        """
        logger.info(f"Starting Deezer download for ISRC: {isrc}")
        ...
        try:
            ...
            track_data = self.api_client.get_track_by_isrc(isrc)
            metadata = DeezerTrackMetadata.from_api_response(track_data)
            
            # Step 2: Get download URL
            download_url = self.api_client.get_download_url(
                metadata.deezer_id,
                quality="flac"
            )
            ...
            if metadata.cover_url:
                cover_data = self.api_client.download_cover_art(metadata.cover_url)
            ...
            logger.error(f"Download failed for ISRC {isrc}: {e}")
            raise DownloadError(f"Download failed: {e}")


__all__ = ["DeezerDownloader"]
